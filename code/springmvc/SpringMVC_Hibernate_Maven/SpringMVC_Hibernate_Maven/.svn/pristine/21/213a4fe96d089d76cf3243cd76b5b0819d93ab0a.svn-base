...
public class BufferedRandomAccessFile extends RandomAccessFile {
    ...
    static ResourceBundle res = ResourceBundle.getBundle("com.lh.common.Res");
    ...
    protected static MappedByteBuffer mapBuf = null;
    ...
    /**
     * 读取文件放入集合里面
     * @param fromfname 	目录地址
     * @throws IOException
     */
    public static void read2list(String fromfname) throws IOException{
        BufferedRandomAccessFile brafReadFile;
        brafReadFile = new BufferedRandomAccessFile(fromfname);
        byte buf[] = new byte[Constants.LEN_BLOCK];
        int readcount;
        Constants.SHELLList.clear();
        while((readcount = brafReadFile.read(buf)) != -1) {
        	byte b[] = new byte[readcount];
        	System.arraycopy(buf, 0, b, 0, readcount);
        	Constants.SHELLList.add(b);
        }
        brafReadFile.close();
    }
